...
const loadHandlebars = require('./load-handlebars')
...
const epubBuildOptions = {
  ...commonOptions,
  outdir: epubOutDir,
  plugins: [epubOnStartPlugin, loadHandlebars],
  entryPoints: [
    'js/entry/epub.js',
...
const htmlBuildOptions = {
  ...commonOptions,
  outdir: htmlOutDir,
  plugins: [htmlOnStartPlugin, loadHandlebars],
  entryPoints: [
    'js/entry/html.js',
...
function generateDocs (formatter) {
  ...
  process.chdir('../')
  cp.execSync('mix compile --force')
  cp.execSync(`mix docs --formatter ${formatter}`)
