defmodule ExDoc.ModuleData do
  ...
  alias ExDoc.{Config, GroupMatcher}
  ...
  defp do_generate_node(module, module_data, config = %Config{source_url_pattern: source_url}) do
    ...
    %{docs: {:docs_v1, anno, _, _, moduledoc, metadata, _}} = module_data
  ...
  defp source_path(module, config) do
    ...
    if root = config.source_root,
      do:   Path.relative_to(source, root),
      else: source
  end
  ...
  defp doc?({{kind, _, _}, _, _, _, _}, _)
    when kind not in [:function, :macro],           do: false
  # Skip impl_for and impl_for! for protocols
  defp doc?({{_, name, _}, _, _, :none, _}, :protocol)
    when name in [:impl_for, :impl_for!],           do: false
  # Skip docs explicitly marked as hidden
  defp doc?({_, _, _, :hidden, _}, _),              do: false
  # Skip default docs if starting with _
  defp doc?({{_, name, _}, _, _, :none, _}, _type), do: hd(Atom.to_charlist(name)) != ?_
  # Everything else is ok
  defp doc?(_, _),                                  do: true
  ...
  defp get_function(function, source, module_data, groups_for_functions) do
    ...
    specs      = module_data.specs
                 |> Map.get(actual_def, [])
                 |> Enum.map(&Code.Typespec.spec_to_quoted(name, &1))
    specs      = if type == :macro, do:   Enum.map(specs, &remove_first_macro_arg/1),
                                    else: specs
    ...
    %ExDoc.FunctionNode{
      ...
      source_url: source_link(source, find_function_line(module_data, actual_def) || doc_line),
      type: type,
      group: Enum.find_value(groups_for_functions, fn {group, filter} -> filter.(metadata) && group end),
  ...
  defp actual_def(name, arity, :macrocallback),
    do: {String.to_atom("MACRO-" <> to_string(name)), arity + 1}

  defp actual_def(name, arity, :macro),
    do: {String.to_atom("MACRO-" <> to_string(name)), arity + 1}
  ...
  defp module_title_and_id(%{name: module, type: :task}),
    do: {task_name(module), module_id(module)}
