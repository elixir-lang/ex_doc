defmodule ExDoc.HTMLFormatter do
  ...
  def run(modules, config)  do
    proj_output = Path.join(config.output, to_string(config.project))

    generate_project_index(proj_output, config)
  ...
  def create_index(config, proj_infos) do
    # Due to the order of fields in ExDoc.ProjectInfo sort does the right thing.
    content = Templates.master_index_template(Enum.sort(proj_infos))
    File.write("#{config.output}/index.html", content)
  end

  # To make things nice and confusing this is the index for a specific project,
  # not the index of all projects. That index is generated by `create_index`.
  defp generate_project_index(output, config) do
    content = Templates.project_index_template(config)
  ...
  # The HtmlFormatter requires an output dir to be present.
  def needs_output_dir, do: true
end
