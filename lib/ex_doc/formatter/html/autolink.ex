defmodule ExDoc.Formatter.HTML.Autolink do
  ...
  `doc_string` is the string to transform. If `doc_string` is `nil`, it returns `nil`.

  `id` can be either:
    - a string representing the module or the mfa
    - `nil`
    - a tuple in the shape of `{id, node_type}` where `id` is a string
      representing the module or the mfa, and `node_type` is an atom,
      representing the node type: `:module`, `:function`, `:type`, etc.

  `compiled` is a map generated by `compile/3`.
  """
  def project_doc(doc_string, id, compiled)
  def project_doc(nil, _id, _compiled), do: nil

  def project_doc(string, {id, node_type}, compiled) when is_atom(node_type) do
    project_doc(string, id, node_type, compiled)
  end

  def project_doc(string, id, compiled) do
    project_doc(string, id, :module, compiled)
  end

  defp project_doc(string, id, node_type, compiled) when is_binary(string) and is_map(compiled) do
    config =
      compiled
      |> Map.put(:id, id)
      |> Map.put_new(:node_type, node_type)
  ...
  defp id(%{id: id}, nil), do: {id, :module}

  defp id(%{id: mod_id}, %ExDoc.FunctionNode{id: id, type: :callback} = node),
    do: {"c:#{mod_id}.#{id}", node.type}

  defp id(%{id: mod_id}, %ExDoc.FunctionNode{id: id} = node), do: {"#{mod_id}.#{id}", node.type}
  defp id(%{id: mod_id}, %ExDoc.TypeNode{id: id} = node), do: {"t:#{mod_id}.#{id}", node.type}
    ...
    fn all, text, match ->
      ...
      cond do
        match == module_id ->
          case link_type do
            :normal ->
              if options[:node_type] == :module do
                "`#{match}`"
              else
                "[#{match}](#content)"
              end
  ...
  defp replace_fun(:function, :elixir, link_type, options) do
    ...
    fn all, text, match ->
      ...
      id_pmfa =
        {id_prefix, _id_module, id_function, id_arity} = split_match(:module, options[:id])

      text = default_text("", link_type, pmfa, text)

      cond do
        match in locals ->
          if link_type == :normal and
               {prefix, function, arity} == {id_prefix, id_function, id_arity} do
            "#{text}"
          else
        ...
        match in docs_refs ->
          if link_type == :normal and pmfa == id_pmfa do
            "#{text}"
          else
