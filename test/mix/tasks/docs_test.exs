...
defmodule Mix.Tasks.DocsTest do
  ...
  defp run_task(context, args, opts, generator) do
    opts = Keyword.put_new(opts, :output, context[:tmp_dir])

    # TODO: Use with_io on Elixir v1.13
    io =
      capture_io(fn ->
        send(self(), Mix.Tasks.Docs.run(args, opts, generator))
      end)
  ...
  test "accepts output in :output", %{tmp_dir: tmp_dir} = context do
    [{:ok, {_, _, html_options}}, {:ok, {_, _, epub_options}}] =
  ...
  test "accepts options from root", context do
    ...
    assert [{:ok, {"ex_doc", "dev", _}}, {:ok, {"ex_doc", "dev", _}}] =
  ...
  test "exits with 1 due to warnings, with flag --warnings_as_errors", context do
    ...
    output =
      capture_io(:stderr, fn ->
        assert catch_exit(run(context, [], app: :single, docs: [warnings_as_errors: true])) ==
                 {:shutdown, 1}
      end)

    assert output =~
             "Documents have been generated, but generation for html, epub formats failed due to warnings " <>
               "while using the --warnings-as-errors option."
  end
end
