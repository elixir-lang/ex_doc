defmodule ExDoc.HighlighterTest do
  ...
  test "if no language is given, highlight as iex/elixir" do
    ...
    assert Highlighter.highlight_code_blocks(with_empty_class) =~ ~r{<pre><code class=\"nohighlight makeup elixir\">.*}
    assert Highlighter.highlight_code_blocks(without_class) =~ ~r{<pre><code class=\"nohighlight makeup elixir\">.*}
    # IEx is highlighted by the normal elixir lexer
    assert Highlighter.highlight_code_blocks(iex_detected_with_empty_class) =~ ~r{<pre><code class=\"nohighlight makeup elixir\">.*}
    assert Highlighter.highlight_code_blocks(iex_detected_without_class) =~ ~r{<pre><code class=\"nohighlight makeup elixir\">.*}
  end
end
