defmodule ExDoc.Retriever.ErlangTest do
  ...
  defp default_config() do
    %ExDoc.Config{source_url_pattern: fn path, line -> "#{path}:#{line}" end}
  end
  ...
  describe "docs_from_modules/2" do
    ...
    test "module with no docs is generated", c do
      ...
      assert {[_], []} = Retriever.docs_from_modules([:mod], default_config())
    end
    ...
    test "module with no chunk", c do
      ...
      assert {[], []} = Retriever.docs_from_modules([:no_chunk], default_config())
    end
    ...
    test "types", c do
      ...
      assert hd(opaque1.specs) |> Erlang.autolink_spec(current_kfa: {:type, :opaque1, 0}) ==
               "opaque1()"
      ...
      assert hd(nominal1.specs) |> Erlang.autolink_spec(current_kfa: {:type, :nominal1, 0}) ==
      ...
      assert hd(type1.specs) |> Erlang.autolink_spec(current_kfa: {:type, :type1, 0}) ==
