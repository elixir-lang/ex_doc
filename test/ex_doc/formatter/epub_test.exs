defmodule ExDoc.Formatter.EPUBTest do
  ...
  defp generate_docs_and_unzip(tmp_dir, config) do
    generate_docs(config)
    unzip_dir = String.to_charlist("#{doc_config(tmp_dir)[:output]}")

    "#{doc_config(tmp_dir)[:output]}/#{doc_config(tmp_dir)[:project]}.epub"
    |> String.to_charlist()
    |> :zip.unzip(cwd: unzip_dir)
  end

  test "generates headers for module pages", %{tmp_dir: tmp_dir} do
    generate_docs_and_unzip(tmp_dir, doc_config(tmp_dir, main: "RandomError"))
  ...
  test "allows to set the primary language of the document", %{tmp_dir: tmp_dir} do
    generate_docs_and_unzip(tmp_dir, doc_config(tmp_dir, main: "RandomError", language: "fr"))

    content = File.read!(tmp_dir <> "/epub/OEBPS/RandomError.xhtml")
    assert content =~ ~r{<html.*lang="fr".*xmlns:epub="http://www.idpf.org/2007/ops">}ms
  end

  test "allows to set the authors of the book", %{tmp_dir: tmp_dir} do
    generate_docs_and_unzip(tmp_dir, doc_config(tmp_dir, authors: ["John Doe", "Jane Doe"]))
  ...
  test "raises when assets are invalid", %{tmp_dir: tmp_dir} do
    ...
    assert_raise(
      ...
      fn -> generate_docs(doc_config(tmp_dir, assets: "test/tmp/epub_assets")) end
    )
  after
    File.rm_rf!("test/tmp/epub_assets")
  end

  test "generates an EPUB file in the default directory", %{tmp_dir: tmp_dir} do
    generate_docs(doc_config(tmp_dir))
    assert File.regular?(tmp_dir <> "/epub/#{doc_config(tmp_dir)[:project]}.epub")
  end

  test "generates an EPUB file with erlang as proglang", %{tmp_dir: tmp_dir} do
    generate_docs(Keyword.put(doc_config(tmp_dir), :proglang, :erlang))
    assert File.regular?(tmp_dir <> "/epub/#{doc_config(tmp_dir)[:project]}.epub")
  end

  test "generates an EPUB file in specified output directory", %{tmp_dir: tmp_dir} do
    config = doc_config(tmp_dir, output: tmp_dir <> "/epub/another_dir", main: "RandomError")
    generate_docs(config)

    assert File.regular?(tmp_dir <> "/epub/another_dir/#{doc_config(tmp_dir)[:project]}.epub")
  end

  test "generates an EPUB file with a standardized structure", %{tmp_dir: tmp_dir} do
    generate_docs_and_unzip(tmp_dir, doc_config(tmp_dir))
  ...
  test "generates the readme file", %{tmp_dir: tmp_dir} do
    config = doc_config(tmp_dir, main: "README")
    generate_docs_and_unzip(tmp_dir, config)
  ...
  test "uses samp as highlight tag for markdown", %{tmp_dir: tmp_dir} do
    generate_docs_and_unzip(tmp_dir, doc_config(tmp_dir))
  ...
  test "before_closing_*_tags required by the user are in the right place", %{tmp_dir: tmp_dir} do
