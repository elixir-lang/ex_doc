defmodule ExDoc.CLITest do
  ...
  test "loading config" do
    ...
    {project, version, opts} = run(["ExDoc", "--extra-title", "Guides", "--extra", "README.md", "1.2.3", "...", "-c", "test.config"])

    assert project == "ExDoc"
    assert version == "1.2.3"
    assert Enum.sort(opts) == [extra_title: "Guides", extras: ["README.md"], formatter_opts: [key: "val"], source_beam: "..."]
  after
    File.rm!("test.config")
  end
